...
def output(
    ...
    out: Union[
        ...
        DataFrame,
        TextIO,
        Tuple[Union[DataFrame, TextIO], Union[DataFrame, TextIO]],
    ],
    fformat: str = None,
):
    """
...
def main():
    ...
    parser.add_argument(
        "-v", "--version", action="version", version=f"scldata {__version__}"
    )
    parser.add_argument(
        "-f",
        "--format",
    ...
    if len(sys.argv) == 1:
        ...
        len(sys.argv) == 3
        and (sys.argv[1] == "--info" or sys.argv[1] == "-i")
        and sys.argv[2] == "head"
    ):
        print(
            'Option "--info/-i head" must be accompanied by "--split/-s", and optionally "--format/-f".'
        )
        sys.exit(0)
    ...
    if args.scls:
        out_manager_scl = OutputManager(
            f"{args.output}-targets-{args.scls}.txt"
            if args.output != "-"
            else args.output
        )
        ...
        if args.split not in ("0", "1", "2", "3", "4"):
            ...
            head_out = (
                StringIO("".join(map(str, load(args.split, form).readlines()[:20])))
                if args.format == "fasta"
                else load(args.split).head()
            )
            output(out_manager, head_out, args.format)
        else:
            print(f"SCL2205 Fold-{args.split} Train-Valid (Head):\n\n")
            head_out1 = (
                StringIO("".join(map(str, load(args.split, form)[0].readlines()[:20])))
                if args.format == "fasta"
                else load(args.split)[0].head()
            )
            head_out2 = (
                StringIO("".join(map(str, load(args.split, form)[1].readlines()[:20])))
                if args.format == "fasta"
                else load(args.split)[1].head()
            )
            output(out_manager2, (head_out1, head_out2), args.format)
        ...
        if args.split not in ("0", "1", "2", "3", "4"):
            output(
                out_manager,
                f"SCL2205 {args.split.capitalize()} Shape:\n{load(args.split).shape}\n\n",
            )
        else:
            output(
                out_manager,
                f"SCL2205 Fold-{args.split} Train Shape:\n{load(args.split)[0].shape}\n",
            )
            output(
                out_manager,
                f"SCL2205 Fold-{args.split} Valid Shape:\n{load(args.split)[1].shape}\n",
            )
