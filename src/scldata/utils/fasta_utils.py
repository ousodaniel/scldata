...
def seq_df_to_fasta_handle(df: pd.DataFrame, header_col: Optional[str]=None, description_col: Optional[str]=None, seq_col: str= 'seq') -> TextIO:
    ...
    `seq_col Column`: `str`, required

    :param df: `pandas.DataFrame`, Sequence DataFrame.
    :param header_col: `str`, Name of column to use as the fasta sequence header if not index/None. By default, the index is the header. All spaces are replaced with underscores.
    :param description_col: `str`, Name of column to use for sequence description, if 'description_col' is not None. By default, no description is used.
    :param seq_col: `str`, Column for the sequence data.

    Returns
    -------
    `StringIO`

    :return: A `StringIO` handle for the corresponding FASTA-formatted file.
    """
    fasta_io = StringIO()
